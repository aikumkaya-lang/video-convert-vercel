...
export default async function handler(req, res) {
  ...
  try {
    ...
    return jsonError(res, 400, "bad_request_body", { detail: String(e) });
  }
  ...
  const outName = sanitizeFilename(
    filename || (url ? new URL(url).pathname.split("/").pop() || "video.mp4" : "video.mp4")
  );
  ...
  try {
    ...
    if (wantCopyfix) {
      try {
        await runFfmpeg(copyfixH264, stdinStream, outPass);
        return;
      } catch {}
    }
    await runFfmpeg(encTelegram, stdinStream, outPass);
  } catch (err) {
    if (!headersSent)
      return jsonError(res, 502, "convert_failed", {
        detail: (err?.log || String(err)).slice(0, 800)
      });
    try {
      res.end();
    } catch {}
